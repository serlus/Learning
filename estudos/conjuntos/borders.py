...
def has_move(board, line, column):
    ...
    Imagine each player starting with a position occupied. So the first can
    occupy any adjancent Empty slot. After that the second player can do the
    same and so on until there is no more Empty slots adjancent to any occupied
    one. 


    Args:
        board ([type]): Matrix m x n
        line ([type]): The line of start slot
        column ([type]): the column of start slot
        :return: True if there is any Empty slot adjacent to the are occupied
        by one player.
...
print(has_move(board_white_finished, 0, 3))  # expected False
